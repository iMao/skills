...
public:
  ...
  Stock(const std::string &company, long shares = 0,
        double share_val = 0.0); //конструктор с несколькими параметрами
  ...
  static void set_counter(
      int init_val); //статическая функция инициализации счетчика объектов
};
...
public:
  ...
  A() : s("Default class A") {
    std::cout << "Default constructor of class A()" << std::endl;
  }
  A(std::string s) : s(s) {
    std::cout << "Constructor of class A(p)" << std::endl;
  }
  A(const A &a) : s(a.s) {
    std::cout << "Copy constructor of class A(A&)" << std::endl;
  }
  A(A &&a) {
    s = std::move(a.s);
...
public:
  ...
  ~OpenBase() { cout << "Destructor OpenBase" << endl; }
...
public:
  ...
  ~PrivateBase() { cout << "Destructor PrivateBase" << endl; }
...
public:
  base() : name("no name"), surname("no surname") {
    cout << "base default constructor" << endl;
  }
  base(string name, string surname) : name(name), surname(surname) {
    cout << "base constructor(p1,p2)" << endl;
  }
...
//открытое наследование класса open_base с областью protected унаследованной от
//класса base
...
//закрытое наследование класса open_base с областью protected унаследованной от
//класса base
class close_base : private open_base {
public:
  close_base() : open_base() {
    cout << "close_base default constructor " << endl;
  }
...
public:
  figure() : visible(true) {
    cout << "default constructor of class figure" << endl;
  }
  ...
  void getStatus() const {
    cout << "Status: " << (visible == true ? "visible" : "invisible") << endl;
  }
};
...
public:
  circle_quad(double a) : circle(a / 2), quad(a) {
    cout << "circle_quad: "
         << "A = " << a << " R = " << a / 2 << endl;
  }
...
public:
  ...
  basic(int i, int j) : i(i), j(j) {
    cout << "constructor of class basic" << endl;
  }
...
public:
  ...
  Char(char c) : c(c) {
    cout << " constructor of class Char "
         << "Char: " << c << '\n';
  }
...
public:
  UpperChar(char c) : Char(c) {
    cout << "constructor of class UpperChar" << endl;
  }
...
public:
  ASCIChar(char c) : Char(c) {
    cout << "constructor of class ASCIChar" << endl;
  }
  ~ASCIChar() { cout << "destructor of class ASCIChar" << endl; }
  void show_char() {
    cout << "Char: "
         << "[" << c << "]"
         << "has ASCI code =  " << static_cast<int>(c) << endl;
  }
};
...
public:
  attributes() {}
  attributes(bbox box, point center)
      : visible(true), box(box), center(center) {}
  ~attributes() {}
...
public:
  Quad() = delete;
  Quad(int a, int x, int y)
      : graph_object(bbox((x - a / 2), (y - a / 2), a, a), point(x, y)), a(a) {
    cout << "Quad: " << a << "x" << a << endl;
  }
  ~Quad() {}
