...
template <typename t, int n> void print_array(std::array<t, n> &a) {
  cout << endl;
  for (auto x : a)
    cout << "x = " << x << endl;
}
...
public:
  ...
  bool operator!=(Object &object) { return !((*this) == (object)); }
...
template <typename t> std::pair<t, t> &solve_equation(t a, t b, t c) {
  t D = b * b - 4 * a * c;
  if (D < 0)
...
// ---------------------------------------
template <typename t> void print_vector_info(std::vector<t> &vec) {
  cout << endl;
...
template <typename t> void vector_empty_info(std::vector<t> &v) {
  cout << endl;
  if (v.empty())
    cout << "v - empty" << endl;
  else
...
template <typename t> void test_iterators(std::vector<t> &vec) {
  cout << endl;
...
// ------------------------------------------------
template <typename t> void print_deque_info(std::deque<t> &dq) {
  cout << endl;
  cout << "size - " << dq.size() << endl;
  cout << "max_size - " << dq.max_size() << endl;
}

template <typename t> void print_deque(std::deque<t> &dq) {
  cout << endl;
  if (dq.empty())
...
class Near {
public:
  bool operator()(double &a, double &b) { return (fabs(a - b) < 0.5); }
};
...
void mapbounds(std::map<Key, Val, Cmp> &mp, Key val) {
  ...
  std::pair<std::map<Key, Val, Cmp>::iterator,
            std::map<Key, Val, Cmp>::iterator>
...
void print_unordered_set_info(std::unordered_set<t, hf, cmp> &uset) {
  cout << endl;
  cout << "Количество сегментов                       - " << uset.bucket_count()
       << endl;
  ...
  cout << "Максимально возможное количество элементов - " << uset.max_size()
       << endl;
  cout << "Текущий коэффициент заполнения             - " << uset.load_factor()
       << endl;
