...
void test_array() {
  ...
  std::transform(flt.begin(), flt.end(), transform_flt.begin(),
                 std::negate<float>()); //результаты положим в другой массив
  print_array(flt);
  print_array(transform_flt);

  std::transform(flt.begin(), flt.end(), flt.begin(), [](float &a) -> float {
    return a *= 2.0f;
  ...
  cout << endl
       << ((flt.empty() == true) ? " array is empty" : " array full") << endl;
  cout << endl << " flt array max size = " << flt.max_size() << endl;
  cout << endl
       << " half_zero array max size = " << half_zero.max_size() << endl;
...
void test_pairs() {
  ...
  std::pair<string, string> a3 = std::make_pair(
      std::move(f), std::move(s)); //использование семантики перемещения в
...
void test_tuples() {
  ...
  std::tuple<int, float, string>
      t0; //кортеж из 3-х элементов с инициализацией конструкторами по умолчанию
  std::tuple<int, float, string> t1(
      5, 4.2f,
      "string"); //кортеж из 3-х элементов с инициализацией заданными значениями
  std::tuple<int, float, string> t2{
      3, 2.71f,
      "Hello"}; //кортеж из 3-х элементов с инициализацией списком инициализации
  std::tuple<int, float, string> t3(
      t1); //кортеж из 3-х элементов с инициализацией конструктором копирования
  auto t4 = std::make_tuple(0, 0.1f,
                            string("auto")); //создание кортежа с выводом типов
  ...
  cout << endl
       ...
       << std::get<2>(t7) << "," << std::get<3>(t7) << "," << std::get<4>(t7)
       << "," << std::get<5>(t7) << "]" << endl;

  std::pair<int, string> p0{9, std::string("pair")};
  std::tuple<int, string> t8(p0); //создание кортежа путем вызова конструктора
...
template <> void print_vector<bool>(std::vector<bool> &v) {
  cout << endl;
  for (auto &e : v)
...
void test_vector() {
  ...
  std::vector<int> vec2(
      3, 777); //создание вектора из 3-х элементов со значениями 777, 777, 777
  std::vector<int> vec3{
      1, 0, 35,
  ...
  std::vector<int> vec5(
      std::move(vec3)); //создание вектора при помощи перемещающего конструктора
  ...
  vec5.reserve(
      10); //увеличиваем размер памяти для вектора для хранения 10 элементов
  ...
  vector_empty_info(
      vec3); //применение конструктора перемещения оставило вектор vec3 - пустым

  //присваивание элементов
  std::vector<int> vec10 = initList;
  print_vector(vec10);

  vec10.assign({0, 0, 0}); // assign - это просто присвоение, при этом удаляются
                           // все прежние элементы вектора
  print_vector(vec10);
  ...
  cout << endl
       << "front e = " << vec10.front() << " back e = " << vec10.back() << endl;
  ...
  int_vec.insert(int_vec.begin(), ivec.begin(),
                 ivec.end()); //вставляем диапазон значений из другого вектора
  print_vector(int_vec);
...
void test_deque() {
  ...
  std::deque<string> dq1(3); //контейнер на 3 элемента инициализированных своими
  ...
  std::deque<string> dq6((++dq5.cbegin()),
                         dq5.cend()); //создание контейнера на основе диапазоне
  ...
  std::deque<string> dq8 =
      dq2; //создание контейнера на основе оператора копирующего присваивания
  std::deque<string> dq9 = std::move(
      dq5); //создание контейнера на оснопе оператора перемещающего присваивания
...
void test_list() {
  ...
  std::list<int> lst7(lst3.begin(),
                      lst3.end()); //список создан на основе интервала заданного
                                   //итераторами (begin, end)
...
void test_list_special() {
  ...
  lp.merge(lp2, compare_pairs); //объединяю два отсортированных списка на основе
                                //функции-предиката
  print_list(lp);
}
...
void test_forward_list() {
  ...
  std::forward_list<string> strlist2{"hello",   "HELLO",        "BAY-BAY",
                                     "bay-bay", "good morning", "GOOD MORNING"};
...
void test_sets() {
  ...
  std::set<int, std::less<int>> set2; //пустое множество элементов типа int, с
                                      //явно заданным критерием сортировки
  std::set<int, std::greater<int>> set3; //пустое множество элементов типа int,
                                         //с явно заданным критерием сортировки

  //	std::set<int> set4(std::less<int>());				//пустое
  //множество элементов типа int, но критерий сортировки задан как параметр в
  //конструкторе

  std::initializer_list<int> ilst{9, 3, 10, 2, 5, 1, 0, 7};

  std::set<int, std::less<int>> set5(
      ilst); //множество элементов типа int c заданным критерием сортровки,
             //инициализированное списком инициализации
  std::set<int> set6(set5); //множество созданное конструктором копирования
  std::set<int> set7(
      std::move(set5)); //множество созданное конструктором перемещения

  std::list<int> list0{8, 5, 6, 1, 4, 2, 9, 0};
  std::set<int> set8(
      list0.begin(),
      list0.end()); //множество элементов созданное на основе элементов из
                    //интервала заданного итераторами, с критерием сортировки по
                    //умолчанию

  std::set<int, std::greater<int>> set9(
      set8.begin(), set8.end(),
      std::greater<int>()); //множество созданное на основе элементов из
                            //интервала другого множества но с другим критерием
                            //сортировки
...
void test_maps() {
  //методы создания
  std::map<int, string>
      mp0; //пустое отображение с элементами std::pair<int, string> и критерием
           //упорядочивания по умолчанию std::less<>
  std::map<int, string, std::greater<int>> mp1;

  std::map<int, string> mp2{std::make_pair(5, "Tom"), std::make_pair(8, "Jack"),
  ...
  std::initializer_list<std::pair<const int, string>> ilp{
      std::make_pair(4, string("Andrew")), std::make_pair(9, string("Timur")),
      std::make_pair(2, string("Nico")), std::make_pair(6, string("Peter"))};
  ...
  cout << endl
       << "Find element wih key 3, val = " << (*mp12.find(3)).second << endl;
  ...
  cout << endl;
  auto pos =
      std::find_if(pp.begin(), pp.end(), [](std::pair<const string, float> &p) {
...
void test_unordered_sets() {
  ...
  cout << endl
       << "Находим интервал для элемента 1 " << *(uset1.equal_range(1).first)
...
void cin_properties(std::istream &cin_) {
  ...
  cout << endl
       << "проверка битов состояния с помощью функций good(), eof(), fail(), "
          "bad() "
       << endl;
  ...
  cout << endl
       << "Проверка битов состояний которые генерируют исключения" << endl;
...
void test_ithreads() {
  ...
  // символов, \n - оставляет в потоке cout << endl << "You input: " << buffer
  // << endl;

  // cout << endl << "input string - ";
  // cin.get(buffer, 9, '-');
  // //функция istream & istream::get(char*, streamsize n, char delim) cout <<
  ...
  // извлекается из потока но не вставляется в буфер cout << endl << "You input:
  // " << buffer << endl;

  // memset(buffer, '\0', N);
  // cout << endl << "Input string - ";
  // cin.getline(buffer, 6, '\\');
  // //функция istream& istream::getline(char*, streamsize n, char delim) cout
  // << endl << "You input: " << buffer << endl;
  ...
  // количество прочитанных символов cout << "You input: " <<countchar<<" chars
  // "<< buffer
  // << endl;
  //
  // int gc = cin.gcount();
  ...
  // 234 356 cin.get(buffer, 10); cout << "You input: " << buffer << std::endl;

  // cout << "Input char chain with \' \'" << std::ends;
  // //добавляем символ \0 в конец вывода cin >> std::ws;
  // //включаем игнорирование пробельных символов cin.get(buffer2, 10); cout <<
  ...
  // cout << '|' << std::setw(15) << std::showpos << std::right <<
  // std::setfill(' ') << num
  //	 << '|' << std::setw(15) << std::showpos << std::right << std::setfill('
  //') << num << '|' << endl;
  ...
  // cout << '|' << std::setw(15) << std::showpos << std::right << std::hexfloat
  // << num
  //		 << '|' << std::setw(15) << std::showpos << std::right <<
  ...
  ///знаков числа
  // cout << '|' << std::setw(15) <<std::setprecision(3) << std::defaultfloat <<
  // std::showpos << std::right << num
  //	<< '|' << std::setw(15) << std::setprecision(4) << std::defaultfloat <<
  ...
  int intnum = 255;
  // cout << endl;
  // cout << '|' << std::setw(17) << std::dec << std::showpos << std::right <<
  // intnum << '|' << endl; cout << '|' << std::setw(17) << std::hex <<
  // std::showpos << std::right << intnum << '|' << endl; cout << '|' <<
  // std::setw(17) << std::uppercase<< std::hex << std::showpos << std::right <<
  // intnum << '|' << endl; cout << '|' << std::setw(17) << std::oct <<
  // std::showpos << std::right << intnum << '|' << endl; cout << '|' <<
  // std::setw(17) << std::dec <<std::showbase<< std::showpos << std::right <<
  // intnum << '|' << endl;
  ...
  cout << myendl;
  cout << "Hello ostream" << myendl << "I am glad to see You again" << myendl
       << "I am happy" << myendl;
  cout << "Привет ostream" << myendl << "Я рад видеть тебя снова" << myendl
       << "Я счастлив" << myendl;
