...
void TestArray() {
  ...
  //результаты положим в другой массив
  std::transform(flt.begin(), flt.end(), transform_flt.begin(),
                 std::negate<float>());
  PrintArray(flt);
  PrintArray(transform_flt);
...
//      3, 2.71f, "Hello"};  //кортеж из 3-х элементов с инициализацией списком
//                           //инициализации
...
//  cout << endl;
//  for (auto &e : v) cout << "bit = " << std::boolalpha << e << endl;
//}
...
//      std::move(dq5);  //создание контейнера на оснопе оператора перемещающего
//                       //присваивания
...
//  std::list<int> lst1(5);  //список из 5-ти элементов созданных конструктором
//  их
//                           //типа по умолчанию
...
//  b.splice(
//      ++b.begin(), a, ++a.begin(),
//      a.end());  //перемещаем все элементы кроме первого из списка а в список
//      b
//  print_list(a);
//  print_list(b);
...
//  std::set<int, std::less<int>> set2;  //пустое множество элементов типа int,
//  с
...
//                     //интервала заданного итераторами, с критерием сортировки
//                     //по умолчанию
...
//  //методы создания
//  std::map<int, string>
//      mp0;  //пустое отображение с элементами std::pair<int, string> и
//      критерием
...
//  std::tuple<string &, int &, float &, char &> tp = std::tie(hello, num, f,
//  ch);
